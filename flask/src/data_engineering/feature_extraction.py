...
class Article(object):
    ...
    def get_features_frog(self):
        ...
        features['question_marks_perc'] = features['question_marks'] / float(token_count)
        features['exclamation_marks'] = clean_ocr.count('!')
        features['exclamation_marks_perc'] = features['exclamation_marks'] / float(token_count)
        currency_symbols = 0
        ...
        features['digits_perc'] = features['digits'] / float(token_count)
    ...
    def get_pruned_features_frog(self):
        '''
        Calculate article features.
        '''
        features = {}
    ...
    def get_pruned_features_spacy(self):
        spacy_nl = spacy.load('nl')
        '''
        Calculate article features.
        '''
        ...
        modal_verb = [t for t in tokens if t[1].startswith('VERB') and
                      t[2].capitalize() in Utilities.modal_verbs]
        features['modal_verbs_perc'] = len(modal_verb) / float(token_count)

        modal_adverb = [t for t in tokens if t[1].startswith('ADV')
            and t[2].capitalize() in Utilities.modal_adverbs]
        features['modal_adverbs_perc'] = len(modal_adverb) / float(token_count)

        cogn_verb = [t for t in tokens if t[1].startswith('VERB') and
            t[2].capitalize() in Utilities.cogn_verbs]
        features['cogn_verbs_perc'] = len(cogn_verb) / float(token_count)

        intensifier = [t for t in tokens if t[0].capitalize() in
            Utilities.intensifiers]
        features['intensifiers_perc'] = len(intensifier) / float(token_count)
